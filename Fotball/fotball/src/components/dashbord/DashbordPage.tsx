...
const DashboardPage = () => {
  ...
  const loadData = async () => {
    ...
    if (financeResponse.success && financeResponse.data && financeResponse.data.length > 0) {
      ...
      // Tving til number i tilfelle noe rart
  ...
  // Ta opp lån via POST /api/Finance/loan
  const handleLoan = async () => {
    ...
    if (response.success && response.data) {
      // Backend returnerer oppdatert finance-objekt
      const f = response.data;
      const cleaned: IFinance = {
        id: f.id,
        moneyLeft: Number(f.moneyLeft),
        numberOfPurchases: Number(f.numberOfPurchases),
        moneySpent: Number(f.moneySpent),
      ...
      setMessage("Kunne ikke ta opp lån.");
    }
  };

  // Kjøp spiller – bruker PUT /api/Finance og PUT /api/Athlete
